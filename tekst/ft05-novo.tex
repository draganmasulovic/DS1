...
\begin{EX} (\emph{Osobine simbola $\sum$})
  ...
  $$
    ...
    \sum_{i=1}^n k \cdot a_i = k \cdot \sum_{i=1}^n.
  $$
\end{EX}
\begin{proof}[Dokaz]
...
\begin{EX}\label{ft05.ex.pitagorine-3}
  ...
  da koliko ima \enquote{esencijalno razli\v citih} pravouglih trouglova sa celobrojnim stranicama.
...
\begin{proof}[Dokaz]
 ...
 broj deljiv sa 1 i samim sobom. Ako broj ima i neki drugi delilac onda nije prost, a takav delilac se naziva \emph{netrivijalanim.}
 Na primer, broj 527 pored \emph{trivijanih delilaca} 1 i 527 ima i \emph{netrivijalni delilac} 17. Zato 527 nije prost broj.
 ...
 \end{itemize}
 Dakle, svaki broj u gornjem nizu ima netrivijalan delilac, pa nijedan nije prost. Eto \enquote{rupe} u nizu prostoh brojeva \v cija du\v zina je~$n$.
\end{proof}
...
poku\v samo da \enquote{direktnim rezonovanjem} do\dj emo do zaklju\v cka~$q$. Primere ovakvih dokaza
...
\begin{proof}[Dokaz]
  ...
  $$
    a^k - 1 = (a - 1)(a^{k-1} + a^{k-2} + \ldots + a + 1)
  $$
  (koja se lako pokazuje direktnim ra\v cunanjem) dobijamo
  $$
...
Na Sl.~\ref{ft01.fig.1potez1}~$(b)$ prikazan je de\v ciji crte\v z koji je nartan tako da mu se grane $AC$ i $BD$ seku.
...
Sl.~\ref{ft01.fig.1potez1}~$(a)$ i $(b)$ nakon kra\'ceg razmi\v sljanja zaklju\v cujemo da je za njihovo crtanje
...
neparno mnogo grana. Na primer, kod crte\v za na Sl.~\ref{ft01.fig.1potez2}~$(a)$ svi \v cvorovi su neparni (iz svakog izlaze tri grane),
crte\v z na Sl.~\ref{ft01.fig.1potez2}~$(b)$ ima dva neparna i tri parna \v cvora ($B$ i $C$ su neparni, dok su $A$, $D$ i $E$ parni),
a kod crte\v za na Sl.~\ref{ft01.fig.1potez2}~$(c)$ svi \v cvorovi su parni.
...
\begin{itemize}
  ...
  \item ako je crte\v z povezan i nema naprnih \v cvorova ili ima ta\v cno dva neparna \v cvora, onda on mo\v ze da se nacrta iz jednog poteza.
\end{itemize}
...
\begin{proof}[Dokaz]
 ...
 $$
   ...
   \;\;\text{\qquad i\qquad}\;\;
   \underbrace{a = b}_q \;\;\Rightarrow\;\; \underbrace{\frac{a+b}{2} = \sqrt{ab}}_q.
 $$
...
\begin{proof}[Dokaz (Kriterijuma deljivosti sa 17)]
  ...
  $$
    ...
    \;\;\text{\qquad i\qquad}\;\;
    \underbrace{17 \mid M - 5c}_q \;\;\Rightarrow\;\; \underbrace{17 \mid N}_q
  $$
...
$$
zna\v ci da je $M$ prirodan broju \v cije cifre u decimalnom zapisu su, sleva udesno, $a_n$, $a_{n-1}$, \ldots, $a_0$.
Drugim re\v cima,
$$
  N = \overline{a_n a_{n-1} \ldots a_1 a_0}
$$
...
\section{Intermezzo: RSA kriptosistem}
  ...
  za \v sifrovanje poruka se koristi par brojeva koji se objavi, to je \emph{javni klju\v c},
  ...
  jer ako su parametri za kriptovanje odabrani na pravi na\v cin tajni klju\v c (koji nam treba da de\v sifrujemo
  ...
  \begin{COR}\label{ft05.posl.mft}
    Neka su $p$ i $q$ razli\v citi prosti brojevi i $a$ prirodan broj takav da je $p \nmid a$ i $q \nmid a$. Tada je
    $$
  ...
  $$
  jer je $k = (p-1)\cdot(q-1)$. Da bismo zavr\v sili dokaz treba jo\v s da poka\v zemo da je
  $$
  ...
  \item sada je $k = \mathit{NZS}(88, 96) = 1056$;
  \item odaberemo $e$ tako da je $1 < e < 1056$ i $\mathit{NZD}(e, k) = 1$, recimo, $e = 17$;
  \item izra\v cunamo $d$ tako da je $d \cdot e \equiv_{1056} 1$, pa dobijamo $d = 497$;
  ...
  To je 6-bitni kod koji mo\v ze da opi\v se 64 simbola prema slede\'coj tabeli:
  \begin{center}
  ...
  Obrnuto, kod 110100 korira simbol \verb`'T'`.
  ...
  \end{flushleft}
  (Ovakav izbor veli\v cine paketa je pogodan i iz jednog drugog razloga: ako je se veli\v cina paketa poklapa da du\v zinom
  kodne re\v ci, \v sto je u ovom slu\v caju 6, ili sa nekim njenim umno\v skom, na primer 12, onda se dobijena \v sifrovana
...
\emph{Indukivno zaklju\v civanje} je su\v sta suprotnost \emph{deduktivnom zaklju\v civanju}:
...
bi nas navelo na hipotezu da je $f(n)$ prost broj za svako $n$, jer ako smo isprobali ovoliko
slu\v cajeva (\v cak 80) i uvek dobili prost broj, sve ukazuje na na to da \'ce
...
Nemojte dozvoliti da vas ime zavara: iako li\v ci na eksperimetisanje tipi\v cno za induktivno
...
\begin{EX}
  Sada \'cemo indukcijom \enquote{dokazati} da su svi prirodni brojevi jednaki. Gde je gre\v ska u \enquote{dokazu}?
\end{EX}
...
\end{EX}
\begin{proof}[Dokaz]
  Tvr\dj enje dokazujemo totalnom indukcijom po broju gradova i dr\v zavi.
  \begin{itemize}
    ...
    \item \emph{Induktivni korak.} Posmatrajmo dr\v zavu koja ima $n$ gradova.
      ...
      \end{center}
      Nadovezivanjem ove dve rute preko grada $G$ dobijamo jednosmernu rutu koja prolazi kroz sve gradove cele dr\v zave.
...
Podsetimo se da indukcija ne mora da po\v cinje od 1, kao i da mo\v ze da se desi da je potrebno proveriti
...
\end{EX}
\begin{proof}[Dokaz]
  Tvr\dj enje dokazujemo totalnom indukcijom.
  \begin{itemize}
  \item
...
\section{Matemati\v cka indukcija u analizi programskog koda}

Sada \'cemo na dva primera pokazati kako se matemati\v cka indukcija koristi u formalnoj analizi programskog koda.
Vide\'cemo da pri razmatranju pona\v sanja \enquote{for}-ciklusa obi\v cno koristi matemati\v cka indukcija \enquote{sa $n$ na $n+1$},
...
promenljiva \verb`rez` sadr\v zi broj $x^n$. Dokaz je veoma jednostavan.
  \begin{itemize}
  \item
    \emph{Baza indukcije.} Za $n = 0$ \verb`for`-ciklus se ne\'ce izvri\v siti nijednom, pa vrednost
...
\item $\displaystyle{1^3+2^3+...+n^3=\Big(\frac{n(n+1)}2\Big)^2}$;
...
\ZAD{04.zad.21}{
Ivica sla\v ze slagalicu. Slagalica se sla\v ze tako \v sto se spajaju dva deli\' ca slagalice tako da oni sa\v cine blok, dodavanjem komada slagalice ve\' cem bloku, ili spajanjem dva bloka. Ako Ivicina slagalica ima 500 deli\' ca, dokazati da \' ce je Ivica slo\v ziti u 499 koraka. 
}
