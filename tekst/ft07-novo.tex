...
U apstraktnom smislu, to zna\v ci da treba da uspostavimo binarnu relaciju
...
Pogledajmo jedan jednostavan primer. Za dva trougla ka\v zemo da su \emph{sli\v cni} ako imaju iste
...
\begin{EX}
    Da se podsetimo: za dva prirodna broja ka\v zemo da su \emph{kongruentni po modulu $m$} i pi\v semo $a \equiv_m b$,
    ako imaju isti ostatak pri deljenju sa $m$. Primetimo da relacija $\equiv_m$ ima tri upravo navedene
    osobine:
    \begin{itemize}
...
\begin{EX}\label{ft07.ex.01-weight-2}
  ...
  da $u$ i $v$ imaju istu te\v zinu (= isti broj jedinica). Relaciju $\sim$ je ilustrovana na Sl.~\ref{ds1.sl.01-4}.
...
\begin{proof}[Dokaz]
  ...
  treba da poka\v zemo da je $[a]_\rho \subseteq [b]_\rho$ i $[b]_\rho \subseteq [a]_\rho$. Pokaza\'cemo samo
...
\begin{proof}[Dokaz]
  ...
  Doka\v zimo da je $\rho$ relacija ekvivalencije na skupu $A$.
    ...
    Recimo da je $a, b \in C_i$ i $b, c \in C_j$. Zato \v sto razli\v citi klasteri nemaju zajedni\v ckih elemenata, zaklju\v cujemo
    da je $C_i = C_j$ jer $b \in C_i \cap C_j$. Dakle, $a$ i $c$ se tako\dj e nalaze u istom klasteru, pa je zato
    $a \mathrel\rho c$.

    Treba jo\v s pokazati da je $A / \Boxed\rho = \{C_1, C_2, \ldots, C_k\}$, ali to sledi direktno iz konstrukcije relacije~$\rho$
...
Algoritam za sortiranje poredi parove podataka po nekom kriterijumu i onda odlu\v cuje koji podatak treba da se pojavi
...
po kom kriterijumu niz treba da bude sortiran. Drugim re\v cima, programer mora da opi\v se relaciju poretka
...
\noindent
koji treba da vrati $-1$ ukoliko je \verb`obj1` strogo manje od \verb`obj2`,
treba da vrati $0$ ukoliko su \verb`obj1` i \verb`obj2` jednaki u odnosu na posmatrano svojstvo,
i treba da vrati $1$ ukoliko je \verb`obj1` strogo ve\'ce od \verb`obj2`.
...
Zove se \emph{leksikografski} jer se ista koristi za ure\dj ivanje re\v ci u leksikonu.
Recimo:
\begin{quote}
  Avala,
  Besna kobila,
...
\begin{itemize}
  ...
  \item ne crtamo linije koje mo\v zemo da dobijemo kao posledicu nartanih linija i \v cinjenice da je relacija tranzitivna.
\end{itemize}
...
\begin{quote}
    ...
    put koji spaja $x$ sa $y$ u kome linije idu stalno navi\v se;
    ako takav put ne postoji, onda su $x$ i $y$ neuporedivi.
\end{quote}
...
Neka je $\sqsubseteq$ parcijalno ure\dj enje kona\v cnog skupa $A$. \emph{Topolo\v sko sortiranje} je proces
kojim se elementi skupa $A$ pore\dj aju u niz
$$
  a_0, a_1, \ldots, a_{n-1}
$$
tako da va\v zi slede\'ce: ako je $x \sqsubseteq y$ onda se $x$ javlja u nizu pre $y$. Na primer,
...
\end{center}
\begin{enumerate}[(a)]
\item Nacrtati graf relacije $\rho$, i
...
\ZAD{08.zad.12}{
Proveriti da li je $\rho$ relacija ekvivalencije, gde je:
\begin{center}
...
\item $A$ je grupa ljudi me\dj u kojima nikoja dvojica nemaju isti broj godina, i nikoja dvojica nisu iste visine. Za osobe $P$ i $Q$ iz skupa $A$ pi\v semo $P\mathrel\rho Q$ akko je
\begin{center}
$\mathrm{visina}(P)\leq\mathrm{visina}(Q)\vee\mathrm{godine}(P)\leq\mathrm{godine}(Q)$;
\end{center}
