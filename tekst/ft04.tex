...
$$
gde je smo slovima $p$, $q$, $r$ i $s$ ozna\v cili slede\'ce iskaze:
...
Krajem XIX veka Gotlob Frege predla\v ze novi jezik matematike koji se danas zove \emph{predikatski ra\v cun}.
...
za precizno uvo\dj enje pojma formule predikatskog ra\v cuna trebati malo vi\v se vremena.
...
\begin{EX}
  Neka su $P$ i $Q$ predikati na skupu $\NN$ prirodnih brojeva dati ovako:
  ...
  \begin{center}
    \textit{za svaki prirodan broj $x$ postoji prirodan $y$ takav da je $x < y$ i $y$ je paran,}
  \end{center}
...
\begin{NAP}\it
  Dakle, ta\v cnost formule zavisi od interpretacije. Kao \v sto smo upravo videli, mo\v ze se desiti da jedna ista formula
...
\textit{Re\v senje.}
    ...
    na krugu kako je to pokazano pored. Tada je svaka izme\dj u one druge dve i to samo treba formalno
    da zapi\v semo.
...
Interesantno je da \emph{ne postoji} predikatska formula sa osobinom da svaki model te formule mora da bude kona\v can skup.
Tako\dj e, \emph{ne postoji} predikatska formula sa osobinom da svaki model te formule mora da ima paran broj elemenata.
...
\end{EX}
\begin{proof}[Re\v senje]
  Uo\v cimo slede\'ci predikat na skupu $\RR$ realnih brojeva:
  \medskip
...
\begin{proof}[Re\v senje]
  ...
  Uo\v cimo slede\'ci predikat na skupu $\NN$ prirodnih brojeva:
  \medskip
...
i da nam stavi do znanja da li formula treba da va\v zi za svaku mogu\'cu vrednost te promenljive
...
\begin{EX}
  ...
  Situacija je identi\v cna situaciju u programiranju gde je promenljiva vezana za blok u kome je deklarisana,
...
\begin{EX}
  Postoji mnogo valjanih formula koje ne poti\v cu od tautologija. Neke od najzna\v cajnijih su:
...
\begin{proof}[Re\v senje]
  ...
  $$
  ta\v can za svaki objekat $z$ (Princip isklju\v cenja tre\'ceg).
\end{proof}
...
da se pri transformacijama logi\v cka vredost formule ne menja.
...
\begin{proof}[Re\v senje]
  ...
  $1 < 2 \land 1 < 3$ i pri tome $\lnot 4 < 3$.
\end{proof}
...
\section{Predikatski ra\v cun u analizi programskog koda}

Sada \'cemo na dva slo\v zenija primera pokazati kako se ve\v stine koje smo do sada stekli koriste u formalnoj analizi programskog koda.
...
\begin{proof}[Re\v senje]
  ...
  $$
  Naime, znamo da u ta\v cki (1) va\v zi d je $p = 0$ i $m = A[0]$. S druge strane, uslov $0 \le j \le -1$ u tre\'cem \enquote{delu} formule
...
\end{verbatim}
  ...
  $$
  Lako se vidi da pre nego \v sto metodu \textit{Sort} krene sa izvr\v savanjem \enquote{for}-ciklusa
...
\ZAD{10.zad.6}{
Na\' ci model za slede\' ce predikatske formule:
