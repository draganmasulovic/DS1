...
Za potrebe ovog kursa iskazi ne govore o sva\-ko\-dnev\-nim situacijama, ve\'c o apstraktnim fenomenima tipi\v cnim za matematiku i ra\v cunarstvo.

  \hspace*{1.5em}Nama \'ce posebno biti interesantni iskazi koji govore o stanju memorije tokom izvr\v savanja programa.
  ...
  i pretpostavimo da je tokom izvr\v savanja programa u jednom trenutku stanje u memoriji ra\v cunara izgledalo ovako:
  \begin{center}
  \begin{tabular}{|lr|}
...
\begin{EX}
  ...
  \begin{itemize}
    ...
    \item $(\lnot \lnot(u \lor v) \land (w \Rightarrow (v \Leftrightarrow u)))$ je iskazna formula sa slovima $u$, $v$,$w$.
  \end{itemize}
\end{EX}
...
%  
  ...
  $$
    ...
    \end{array}.
  $$
  Tako smo dobili da je $\top = v(x \lor z) = v(x) \lor v(z) = \bot \lor \bot = \bot$, 
  odnosno, $\top = \bot$, \v sto je nemogu\'ce.
...
\begin{proof}[Re\v senje]
  ...
  Kako formula ima oblik $p_1 \Rightarrow ( \ldots )$, svaka valuacija u kojoj je ona neta\v cna izgleda ovako:
  $$
    v: \begin{array}{c@{\qquad}c@{\qquad}c@{\qquad}c@{\qquad}c}
  ...
  $$
    ...
    \end{array}.
  $$
  Tako smo dobili da je $\top = v(p_1) = \bot$, odnosno, $\top = \bot$, \v sto je nemogu\'ce.
...
\begin{tabular}{ll}
  $\vDash p \land (p \Rightarrow q) \Rightarrow q$  & \emph{Modus ponens}\\
  $\vDash (p \Rightarrow q) \land \lnot q \Rightarrow \lnot p$  & \emph{Modus tollens}\\
...
\else
    ...
    \begin{DEF}
      ...
      $$
      gde je svaka od formula $\alpha_i$, $\beta_j$, \ldots, $\theta_k$ iskazno slovo ili negirano iskazno slovo.
    ...
    \begin{EX}
      Odrediti konjunktivnu i disjunktivnu normalnu formu formule \hbox{$p \Leftrightarrow q$}.
    \end{EX}
    \begin{proof}[Re\v senje]
...
\paragraph{Problem sa terijerima i kometama.}
...
\emph{komete nemaju kovrd\v zav rep}, \v sto je prili\v cno ta\v cno.

\paragraph{Problem sa magarcima i bivolima.}
...
\paragraph{Problem sa svinjama i balonima.}
Iz slede\'cih premisa izvesti odgovaraju\'ci zaklju\v cak:
\begin{enumerate}
\item%1
...
\item%4
  ...
  (ako je luckast i jede jeftine kifle, onda ne mo\v ze da jede na javnom mestu);
\item%5
...
\section{Iskazne formule u analizi programskog k\^oda}
...
pokazati kako se ve\v stine koje smo stekli koriste za analizu toka programskog k\^oda,
detekciju mrtvog k\^oda i analizu petlji. Kako ra\v cunarski programi postaju sve slo\v zeniji
i slo\v zeniji, formalna analiza programskog k\^oda postaje va\v zna ve\v stina za jednog programera
...
\noindent
\v Sta va\v zi u ta\v cki (1) k\^oda?
\end{EX}
\begin{proof}[Re\v senje]
...
\end{EX}
\noindent
\v sta va\v zi u ta\v cki (1) k\^oda, a \v sta u ta\v cki (2)?
\begin{proof}[Re\v senje]
  Ako k\^od stigne u ta\v cku (1) tada je uslov u \enquote{if} ispunjen, pa u ta\v cki (1) va\v zi
  $x \ge 6 \land y < 7$.
  S druge strane, ako je k\^od stigao do ta\v cke (2) uslov u \enquote{if} nije ispunjen, tako da u ta\v cki (2) va\v zi
...
\begin{proof}[Re\v senje]
  ...
  Ako je k\^od stigao u ta\v cku (2), zna\v ci da uslov
  ...
  Ako je k\^od stigao u ta\v cku (3), zna\v ci da uslovi
  ...
  Kona\v cno, ako je k\^od stigao u ta\v cku (4) nijedan od tri uslova nije ispunjen. Zato u ta\v cki (4)
...
\begin{EX}
  Analiza uslova se mo\v ze upotrebiti da se detektuje mrtav k\^od (\v sto je k\^od koji se nikada
  ne\'ce izvr\v siti). U slede\'cem programskom fragmentu detektovati mrtav k\^od:
{\small
\begin{verbatim}
  if(x < 1) {
    // (1)
...
\begin{proof}[Re\v senje]
  ...
  $x \ge 1 \land x \ge 5 \land x < -3$, \v sto nije mogu\'ce! Dakle, k\^od naveden u bloku koji po\v cinje u
  ta\v cki (3) je mrtav k\^od.
...
\begin{proof}[Re\v senje]
  ...
  $$
  Dakle, ovaj fragment razmenjuje sadr\v zaj promenljivih $x$ i $y$.
\end{proof}
...
\begin{proof}[Re\v senje]
  ...
  $$
    x = b \land y = a.
  $$
  Dakle, ovaj fragment razmenjuje sadr\v zaj promenljivih \verb"x" i \verb"y". Ovaj k\^od je samo akademska glavolomka!
...
\noindent
$(a)$ \v Sta va\v zi u ta\v ckama (1) i (2) k\^oda?

\medskip

\noindent
$(b)$ Dokazati da u ta\v cki (3) k\^oda va\v zi $p + u\cdot y = x \cdot y \land u > 0$, a da u ta\v cki (4) va\v zi $p + u\cdot y = x \cdot y \land u \ge 0$.

\medskip

\noindent
$(c)$ \v Sta va\v zi u ta\v cki (5) k\^oda? \v Sta radi metod \verb"MN"?
\end{EX}
\begin{proof}[Re\v senje]
  $(a)$
  Ako metod stigne u ta\v cku (1) to zna\v ci da uslov iza \enquote{if} nije ispunjen. Dakle, u ta\v cki (1)
  ...
  $$
  zato \v sto je $u$ celobrojna promenljiva.
...
koja va\v zi pre ulaska u telo petlje i nakon svakog izvr\v savanja tela petlje.
Ona ne menja svoj oblik tokom rada petlje, odatle ime (ne menja se, \emph{invarijantna} je).
...
\noindent
$(b)$ Dokazati da u ta\v cki (2) k\^oda va\v zi $q \cdot y + r = x \land r \ge y$,
a da u ta\v cki (3) va\v zi $q \cdot y + r = x \land r \ge 0$ (ovo je invarijanta petlje).

\medskip

\noindent
$(c)$ \v Sta va\v zi u ta\v cki (4) k\^oda? \v Sta radi metod \verb"CK"?
...
\begin{proof}[Re\v senje]
  ...
  ra\v cuna koli\v cnik i ostatak pri celobrojnom deljenju $x$ sa $y$, i kao rezultat vra\'ca celobrojni koli\v cnik.
...
\noindent
$(b)$ Dokazati da u ta\v cki (2) k\^oda va\v zi $\NZD(a, b) = \NZD(x, y) \land a > 0 \land b > 0$
...
\noindent
$(c)$ Dokazati da u ta\v cki (4) k\^oda va\v zi $a = \NZD(x, y)$.
...
\begin{proof}[Re\v senje]
  ...
  $(c)$
  U ta\v cki (4) k\^oda va\v zi invarijanta petlje, i uslov za izlazak iz petlje:
  $$
  ...
  $a$ sa $b$, \v sto je uvek manje od stare vrednosti promenljive $b$), i petlja se zavr\v sava kada $b$ padne na~$0$.
\end{proof}
...
\begin{NAP}\it
Za prirodni broj $k$ i cele brojeve $m$ i $n$ pi\v semo $n\equiv_k m$ ako je $n-m$ deljivo brojem $k$. Tada ka\v zemo da su brojevi $n$ i $m$ kongruentni po modulu $k$.
...
\ZAD{02.zad.2}{
Bez sastavljanja tablice istinitosnih vrednosti dokazati da su slede\' ce is\-kaz\-ne formule tautologije:
\begin{enumerate}[(a)]
